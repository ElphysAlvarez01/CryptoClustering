...
### In Step 1, we Import necessary libraries and load data with "coin_id" as the  index column. 
...
- As you can see in the image displaying the sample Top 10 rows "coin-id" is set as the index column.
...
- #### In Step 3, we will be creating a DataFrame with the scaled data, setting the Coin_id Column as the Index. (Data Image)
...
- The purpose of this step is to **initiate the empty list  inertia = []** to store the different values of 'k'.
- inertia will store the sum of squared distances of samples to their closest cluster center. This is used to evaluate clustering quality).
...
### Step 7 includes adding the cluster predictions into a  DataFrame in order to create a scatter plot to visualize it. 
...
```
#### Scatter Plo for K-Means Clusters (ADD IMAGE OF KMEANS CLUSTER HERE)
...
#### PCA Elbow Curve (ADD IMAGE HERE OF PCA ELBOW IMAGE)
...
k_model_copy.plot.scatter(
    ...
    colormap='rainbow')
```
#### Scatter Plot visualizing PCA Clusters (ADD IMAGE HERE)

### In Step 11, we Transpose
